...
class ShellSafetyPlugin(Plugin):
    ...
    输出:
        ShellSafetyPlugin 实例。
    """

    name = "shell_safety"
    version = "0.1.0"
    desc = "拦截 shell 工具中的交互式、破坏性与未确认写命令"

    @on_tool_pre(tool_name="shell")
    async def block_dangerous_shell(self, event: PluginToolHookEvent) -> ToolHookOutcome | None:
        """检查 shell 命令是否安全。

        输入:
            event: PluginToolHookEvent，arguments 中含 command。

        输出:
            危险时返回 decision="deny" 的 ToolHookOutcome；安全时返回 None 放行。
        """
...
def _find_interactive_command(tokens: list[str]) -> str:
    ...
    for token in tokens:
        if Path(token).name in _INTERACTIVE_COMMANDS:
...
def _find_destructive_command(tokens: list[str]) -> str:
    ...
    for token in tokens:
        if Path(token).name in _DESTRUCTIVE_COMMANDS:
            return Path(token).name
    return ""
...
def _sudo_has_non_interactive_option(tokens: list[str]) -> bool:
    ...
    输出:
        True 表示 sudo 会在需要密码时立即失败而不是等待输入。
    """
...
def _dangerous_recursive_rm(tokens: list[str]) -> bool:
    ...
    输出:
        True 表示命令尝试递归删除根目录、当前目录、父目录或通配路径。
    """
