...
收到消息 → 记忆检索 → ReAct 推理 → 工具调用 → 回复。每轮经过 7 个 Phase：BeforeTurn → BeforeReasoning → PromptRender → Reasoning（内含 before_step / after_step）→ AfterReasoning → AfterTurn。

对话中可直接发送以下内置命令：

| 命令 | 作用 |
|---|---|
| `/stop` | 中断当前正在执行的推理，进度会被保存，下次消息可继续 |
| `/status` `/runtime` | 查看运行状态（记忆引擎、工具数、workspace） |
| `/tools` `/tool_status` | 列出所有已注册工具及是否 always-on |
| `/memory_status` `/memorystatus` | 查看当前会话消息已整理 / 未整理数量 |
| `/memory_rollup` | 手动将 PENDING 记忆归档到 MEMORY |
| `/consolidate` | 对当前会话强制执行一次记忆整理 |
...
LLM 可用 `schedule` / `list_schedules` / `cancel_schedule` 工具创建定时任务，任务持久化到 JSON 文件，重启不丢失。直接和 Agent 对话即可添加，例如：
