...
将下面的 prompt 发给商用模型生成 `style__<slug>.html` 文件。
每次只需替换三个占位符：**[风格名]**、**[风格描述]**。
...
## 任务
创建一个 **[风格名]** 的 Jinja2 子模板。
这是旅行攻略网页渲染系统的风格模板之一。

## Jinja2 模板系统说明

你的文件通过 `{% extends "base.html" %}` 继承基础骨架。
你**只负责覆盖以下 block**：

{% block style_vars %} → 定义 CSS 自定义属性（:root 变量）
{% block style_css %} → 风格特有的全部 CSS 规则
{% block header_decor %} → Hero 区域的纯装饰 HTML（可选）
{% block footer_decor %} → Footer 区域的纯装饰 HTML（可选）

你需要与之配合的 DOM 结构（由 base.html 提供）：

- `.hero` — 顶部封面区，内含 `<h1>`（城市名）、`.subtitle`（日期风格）、`.weather-overview`、`.budget-overview`
- `.day-card` — 每日行程卡片，内含 `<h2>`（Day N）、`.day-theme`、`.weather-badge`、`.activity-item`（多个）、`.day-map`、`.hotel-info`
- `.activity-item` — 单个活动，内含 `<h3>` + `.activity-order`、`.activity-time`、`.activity-desc`、`.activity-location`（含链接）、`.transport-info`、`.meal-info`
- `.budget-section` — 预算明细区，内含 `.budget-grid` > `.budget-item` + `.budget-total`
- `.xhs-section` — 小红书攻略区，内含 `.xhs-note`（多个）
- `.tips-section` — 旅行贴士区，内含 `<ul>` > `<li>`
- `.emergency-section` — 紧急信息区
- `.packing-section` — 行李清单区
- `footer` — 页脚
- `.verification-badge.community-match` / `.verification-badge.community-only` — 数据可信度徽章
- 高德交互地图（JS 动态渲染，你只需设置 `.day-map` 样式）

⚠️ 风格一致性约束（关键 —— 防止封面与下方模块断层）：
1. Hero 的风格特征（几何装饰、花纹、边框、肌理、特殊排版手法）必须向下渗透到
   day-card、budget-section、xhs-section、tips-section 等每个模块，用户翻过封面后
   仍能感受到同一风格
2. 每个模块都应有至少一处风格特有的装饰手段，根据风格类型选择：
   - 装饰性框架 / 边框（如双线、花纹、几何切角、斜切角）
   - 模块内嵌的装饰元素（通过 ::before / ::after 伪元素注入小图案、分隔线、角标）
   - 风格特有的纹理 / 背景图案（如纸张肌理、网格线、放射纹）
   - 标题的装饰性前后缀（短线、菱形、符号、编号装饰等）
3. 即使极简 / 日式极简等"少装饰"风格，也应通过留白比例、分割线粗细与间距、
   非对称排列、文字方向等"负装饰"手法让每个模块风格统一，而非仅靠字号颜色区分
4. 禁止出现 hero 极其华丽而下方模块完全素白的断层 —— 这是最常见的失败模式

CSS 自定义属性必须覆盖的变量名（勿改名）：
--color-bg, --color-surface, --color-primary, --color-secondary, --color-accent,
--color-text, --color-text-secondary, --color-text-heading,
--color-border, --color-shadow,
--font-heading, --font-body, --font-mono,
--font-size-h1, --font-size-h2, --font-size-h3, --font-size-body, --font-size-small,
--line-height,
...
⚠️ 注意：
1. 禁止全局元素选择器（`div {}`、`img {}`、`* {}`），必须用 class 选择器
2. 禁止对 `.day-map` 使用 filter、opacity、mix-blend-mode、backdrop-filter——会破坏高德地图 canvas 渲染
3. `{% block style_vars %}` 后第一行必须是 `<style>`，`{% endblock %}` 前最后一行必须是 `</style>`；`{% block style_css %}` 同理。缺少 `<style>` 标签 CSS 不会生效。
4. 必须显式声明容器宽度约束，不能只依赖 Tailwind CDN 的 max-w-2xl 类。在 style_css 块开头添加：
   main, .container, .max-w-2xl { max-width: 720px; margin: 0 auto; }
5. 全页面背景或纹理直接写在 body {} 上，不要定义 .xxx-wrapper / .xxx-container 等空 class；
   页面背景只需覆盖 body { background-color / background-image }，不允许额外创建包装容器
...
## 技术规范
- Tailwind CSS 2.x 已通过 CDN 加载，可以使用 Tailwind 的 utility class
- Font Awesome 6.0 已通过 CDN 加载，可使用任意图标
- 中文字体: Noto Serif SC (衬线) + Noto Sans SC (无衬线) 已加载
- 使用 CSS 变量管理颜色和间距——上面列出的变量名必须全部覆盖
- 响应式设计，max-width 最大为 672px (max-w-2xl)，移动端友好
- 文字必须保持**高可读性**——背景与文字颜色对比度足够
- 信息必须完整展示——天气、预算、交通、小红书推荐、行李清单一个不落
- 永远用中文输出内容描述（CSS 变量名和 class 名保持英文）
- 代码优雅，CSS 体现对细节的极致追求

## 输出格式
文件以 `{% extends "base.html" %}` 为第一行，到最后一个 `{% endblock %}` 结束。
不要输出 markdown 代码块标记，直接输出 HTML 内容。
...
### 05 斯堪的纳维亚 (Scandinavian)

采用斯堪的纳维亚风格设计，体现北欧设计的简约与功能美学。使用纯白背景，配以特定的北欧色调如淡蓝、浅灰、原木色和淡粉。排版应极度克制且有序，使用大量留白，但与极简主义不同，应加入温暖质感。字体选择无衬线几何字体如Futura或Circular，字重轻盈。装饰元素应极少但精心选择，可使用简单几何图案如三角形和线条。图像应明亮、简洁且自然。整体设计应呈现出清爽、实用且温暖的北欧特质，平衡美学与功能性，参考Kinfolk杂志和丹麦设计品牌HAY的产品美学。
...
### 08 后现代解构 (Postmodern Deconstruction)
...
采用孟菲斯风格设计，重现80年代意大利设计运动的前卫美学。必须使用鲜艳且不协调的色彩组合，如亮粉、青绿、鲜黄和橙色。几何形状是核心元素，应大量使用不规则图形、锯齿形、波浪线和彩色网格。排版应活泼且不拘一格，使用几何感强的无衬线字体，可添加阴影或3D效果。装饰元素应包含孟菲斯标志性的彩色条纹、圆点、Z字形和任意形状的色块。图像可使用几何框架或被几何形状切割。整体设计应呈现出夸张、活泼且反传统的视觉冲击，仿佛来自80年代的未来主义想象，参考Ettore Sottsass的作品和MTV早期的视觉风格。
...
### 15 瑞士国际主义解构 (Deconstructed Swiss Style)
...
### 29 德国表现主义 (German Expressionism)
...
1. 从上方"各风格描述速查"中找到目标风格的描述
2. 将通用 Prompt 中的 `[风格名]`、`[风格描述]` 占位符替换
3. 把完整 prompt 发给 Gemini / Deepseek 等模型（使用下来个人感觉Deepseek输出更加稳定优秀）
4. 将返回的 HTML 保存到 `plugins/travel/templates/style__[风格名].html`
5. 用 `test_render.py` 验证
